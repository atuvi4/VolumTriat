...
export default function SupplementsCard() {
  ...
  const configAnabolic = () =>
    openSheet(
      <ManualEntrySheet
        title="Anabolic Masster · dades de l'etiqueta"
        sub="Per presa (50 g amb aigua): calories i proteïna de l'etiqueta. Ho recordaré per registrar-ho amb un toc."
        submitLabel="Desar dades"
        initial={serv ? { name: 'Anabolic Masster', kcal: serv.kcal, protein: serv.protein } : undefined}
  ...
  const logAnabolic = () => {
    ...
    addExtra({
      ...
      note: 'Suplement · gainer amb aigua · presa 50 g (etiqueta)',
    });
  };
  ...
  return (
    <Card title="Suplements" className="mt-3.5">
      <p className="text-[12.5px] text-muted mt-0 mb-2">
        A part dels batuts de menjar. La creatina no suma calories; l'Anabolic Masster (gainer amb aigua) suma kcal i proteïna segons la teva etiqueta.
      </p>
      ...
      <div className="flex items-center justify-between gap-3 py-2.5 border-t border-line">
        <div>
          <div className="font-semibold text-[14.5px]">Anabolic Masster</div>
          <div className="text-[12px] text-faint">
            {serv
              ? `≈ ${serv.kcal} kcal · ${serv.protein} g proteïna / presa (50 g)`
