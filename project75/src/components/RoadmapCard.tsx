 ...
 *  Per defecte mostra molt poc: fase, objectiu i 2 pills.
 *  La microdosi i els 12 mesos queden plegats. No canvia rutina/menú. */
    ...
    <Card title="Roadmap anual" className={className}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <b className="block text-[15px] font-bold">Fase {phase.order} · {phase.name}</b>
          <p className="text-[13.5px] text-muted leading-snug mt-0.5 mb-0">{phase.goal}</p>
          <div className="flex flex-wrap gap-1.5 mt-2">
            {[phase.nutritionShort ?? 'Nutrició', phase.trainingShort ?? 'Entrenament'].map((p) => (
              <span key={p} className="text-[11.5px] font-bold text-accent-strong bg-accent-soft rounded-full px-2.5 py-1">
                {p}
              </span>
            ))}
          </div>
      ...
      {phase.microProgression && phase.microProgression.length > 0 && (
        <details className="mt-3 group">
          <summary className="cursor-pointer text-[13px] font-semibold text-accent list-none flex items-center gap-1">
            <Icon name="chev" size={14} /> Bici/natació en microdosi
          </summary>
          ...
          {phase.notes && phase.notes.length > 0 && (
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {phase.notes.map((n) => (
                <span
                  key={n}
                  className="text-[11.5px] font-semibold text-muted bg-surface2 border border-line rounded-full px-2.5 py-1"
                >
                  {n}
                </span>
              ))}
